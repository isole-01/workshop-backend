...
public class SessionController {
    ...
    @PostMapping("/create/{wk_id}/{user_id}") // user is instructor
    public ResponseEntity<WorkshopSession> createSession(@RequestBody WorkshopSession wks,
                                                         @Nullable @RequestParam String swkids,
        ...
        if (!workshopRepository.existsById(wk_id) ||
        ...
        group.setWorkshopSession(sessionRepository.findById(wks.getId()).get());
        group.setInternalId(0);
        groupRepository.save(group);
        if (swkids != "" && swkids != null) {
            String arr[] = new String[20];
            arr = swkids.split(" ");
            ArrayList<Long> wkids = new ArrayList<>();
            for (String s : arr) {
                wkids.add(Long.parseLong(s));
            }
            ...
            for (Workshop workshop : workshops) {
                ...
                relation.setSession(sessionRepository.findById(wks.getId()).get());
